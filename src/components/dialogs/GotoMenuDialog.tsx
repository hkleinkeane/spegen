...
export function GotoMenuDialog() {
  const show = useStore((s) => s.showGotoMenuDialog);
    ...
    <Modal visible={show} transparent animationType="fade" onRequestClose={closeDialogs}>
      <View style={{ flex: 1, backgroundColor: t.scrim, justifyContent: 'center', padding: 24 }}>
        <View style={{ backgroundColor: t.surface, borderRadius: 12, padding: 20, maxHeight: '85%' }}>
          <Text style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 8 }}>{S.dialogs.gotoMenu.title}</Text>
          <Text style={{ fontSize: 12, color: t.subtext, marginBottom: 8 }}>{S.dialogs.gotoMenu.hint}</Text>
          <ScrollView style={{ maxHeight: 300 }}>
            {menuList.map((m) => (
              <View key={m.id} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 4 }}>
                ...
                <Pressable
                  // Navigate AND close the dialog: previously the modal stayed open over the
                  // board so the jump was invisible and the button appeared to "do nothing".
                  onPress={() => {
                    navigateTo(m.id);
                    closeDialogs();
                  }}
                ...
                >
                  <Text style={{ color: '#FFFFFF' }}>{S.dialogs.gotoMenu.goTo}</Text>
                </Pressable>
              </View>
            ))}
          </ScrollView>
